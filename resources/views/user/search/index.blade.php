        ...
        <form action="{{ route('user.product.search') }}" method="GET" class="col-md-12 mb-4">
            <div class="row">
                <div class="col-md-3 mb-3">
                    <input type="text" name="keyword" class="form-control" placeholder="Tìm kiếm theo từ khóa" value="{{ request()->get('keyword') }}">
                </div>
                <div class="col-md-3 mb-3">
                    <select name="category" class="form-control">
                        <option value="">Chọn danh mục</option>
                        @foreach ($viewData['categories'] as $category)
                            <option value="{{ $category->MaLSP }}" {{ request()->category == $category->MaLSP ? 'selected' : '' }}>
                                {{ $category->TenLSP }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row">
                @foreach ($viewData['products'] as $product)
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            <img src="{{ asset('storage/' . $product->HinhAnh) }}" class="card-img-top" alt="{{ $product->TenSP }}">
                            <div class="card-body">
                                ...
                                <a href="{{ route('user.home.show', $product->Slug) }}" class="btn btn-info">Xem chi tiết</a>
                            </div>
                        </div>
                    </div>
                @endforeach
